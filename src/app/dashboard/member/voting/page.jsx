...
const MemberVotingPage = () => {
    ...
    const isOwnRequest = (voting) => {
        return voting.applicant_id && voting.applicant_id === currentUserId;
    };
    ...
    const hasVoted = (voting) => {
        return voting.votes?.some(v => v.member_id === currentUserId);
    };

    const getMyVote = (voting) => {
        return voting.votes?.find(v => v.member_id === currentUserId);
    };
    ...
    if (loading) return (
        <div className="flex justify-center items-center min-h-[400px]">
            <div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div><p className="mt-4 text-gray-600">Loading votings...</p></div>
        </div>
    );
            ...
            {activeVotings.length > 0 && (
                <div className="space-y-4">
                    ...
                    {activeVotings.map((voting, index) => {
                        ...
                        const ownRequest = isOwnRequest(voting);
                        ...
                        return (
                            <div key={voting._id || index} className={`bg-white rounded-xl border-2 p-5 ${alreadyVoted ? 'border-green-200' : needsMeeting ? 'border-yellow-400' : 'border-blue-200'
                                ...
                                <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                                ...
                                                {ownRequest && (
                                                    <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 flex items-center gap-1">
                                                        <User className="size-3" /> Your Request
                                                    </span>
                                                )}
                                            </div>
                                            {voting.description && (
                                    ...
                                    <div>
                                        {ownRequest ? (
                                            <span className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-full bg-gray-100 text-gray-500 cursor-not-allowed"
                                                title="You cannot vote on your own loan request">
                                                <Ban className="size-3" /> Cannot Vote
                                            </span>
                                        ) : alreadyVoted ? (
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isApproved ? 'bg-green-100' : 'bg-red-100'}`}>
                                                {isApproved ? <CheckCircle2 className="size-5 text-green-600" /> : <XCircle className="size-5 text-red-600" />}
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <h3 className="font-semibold text-gray-900">{voting.title || 'Voting'}</h3>
                                                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${typeBadge.bg} ${typeBadge.text}`}>{typeBadge.label}</span>
                                                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${isApproved ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isApproved ? 'Approved' : 'Rejected'}</span>
                                                </div>
                                                {myVote && (
                                                    <p className="text-xs text-gray-500 mt-1">Your vote: {myVote.vote === 'approve' ? '✓ Approved' : '✗ Denied'}</p>
                                                )}
                                            </div>
                                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => setIsVoteModalOpen(false)} className="p-1 hover:bg-gray-100 rounded-lg"><X className="size-5 text-gray-500" /></button>
                        </div>

                        <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                            <p className="font-medium text-gray-900">{selectedVoting.title}</p>
                            {selectedVoting.description && <p className="text-sm text-gray-500 mt-1">{selectedVoting.description}</p>}
                        </div>

                        <form onSubmit={handleCastVote} className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" onClick={() => setVoteForm(prev => ({ ...prev, vote: 'approve' }))}
                                    className={`p-4 rounded-xl border-2 text-center transition-all ${voteForm.vote === 'approve' ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-green-300'}`}>
                                ...
                                <button type="button" onClick={() => setVoteForm(prev => ({ ...prev, vote: 'deny' }))}
                                    className={`p-4 rounded-xl border-2 text-center transition-all ${voteForm.vote === 'deny' ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-red-300'}`}>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Reason <span className="text-gray-400">(Optional)</span></label>
                                <textarea value={voteForm.reason} onChange={(e) => setVoteForm(prev => ({ ...prev, reason: e.target.value }))}
                                    placeholder="Why are you voting this way?" rows={2}
                                    className="w-full px-4 py-2.5 border border-gray-300 rounded-xl outline-none resize-none" disabled={isSubmitting} />
                            </div>

                            <button type="submit" disabled={isSubmitting || !voteForm.vote}
                                ...
                                {isSubmitting ? <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>Submitting...</> : <><Send className="size-5" />Submit Vote</>}
                            </button>
                        </form>
                    </div>
                </div>
                                    ...
                                    {selectedVoting.votes?.map((vote, i) => (
                                        <div key={vote.member_id || i} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                                            <div className="flex items-center gap-2">
                                                <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600">{(vote.member_name || '?').charAt(0)}</div>
